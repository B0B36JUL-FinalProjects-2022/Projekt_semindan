...
@testset "Titanic.jl" begin
    ...
    @testset "Read CSV" begin
        @test_throws ErrorException read_csv_data("something.csv")
    end

    @testset "Utils" begin
        @testset "Replace" begin
            @test count(ismissing, replace_in_cols(df, :C, missing, 0)[!, :C]) == 0
            @test sort(unique(replace_in_cols(df, [:D, :G], ["a", "b"], ["A", "B"])[!, :D])) == ["A", "B"]
        end
        @testset "Apply" begin
        ...
        @testset "Onehot" begin
            @test length(names(to_onehot(df, :D; remove_original = true))) == length(names(df)) + length(unique(df[!, :D])) - 1
            @test length(names(to_onehot(df, [:D, :G]; remove_original = true))) == length(names(df)) + length(unique(df[!, :D])) + length(unique(df[!, :G])) - 1 - 1
        end
